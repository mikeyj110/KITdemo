...
$(function () {
  ...
  function uploadAttachment(item){
    ...
    if (totalsize<=1000000){
      ...
      var asa = _.clone(ajaxPostSettingsAttachments);
      asa.xhr = function() {
 ...
 // get the form data
    ...
    $.when.apply(null,attachFuncs)
      .done(function(){
       ...
       ajaxPostSettings.data = JSON.stringify(formObj);
        $.ajax(ajaxPostSettings)
        ...
        .fail(function (response) {
          ...
          if (error && error.name==="ValidationError"){
            ...
            var msg = (ajaxPostSettings.headers["x-apikey"] && ajaxPostSettings.headers["x-apikey"].length < 24) ? "Missing API-key": "Server Error";
            alert(msg);
          }
        });
      })
